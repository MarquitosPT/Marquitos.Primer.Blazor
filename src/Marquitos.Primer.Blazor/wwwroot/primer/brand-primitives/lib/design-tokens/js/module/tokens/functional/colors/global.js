/**
 * Do not edit directly
 * Generated on Tue, 28 Jul 2026 20:18:30 GMT
 */

module.exports = {
  brand: {
    color: {
      success: {
        fg: {
          value: "var(--base-color-scale-green-6)",
          dark: "var(--base-color-scale-green-3)",
        },
        emphasis: {
          value: "var(--base-color-scale-green-5)",
          dark: "var(--base-color-scale-green-6)",
        },
        muted: {
          value: "#8CF2A699",
          dark: "#0D302499",
        },
        subtle: {
          value: "var(--base-color-scale-green-1)",
          dark: "#0E422Cd9",
        },
      },
      danger: {
        fg: {
          value: "var(--base-color-scale-red-5)",
          dark: "var(--base-color-scale-red-4)",
        },
        emphasis: {
          value: "var(--base-color-scale-red-5)",
          dark: "var(--base-color-scale-red-5)",
        },
        muted: {
          value: "#FFABA899",
          dark: "#5E021799",
        },
        subtle: {
          value: "var(--base-color-scale-red-0)",
          dark: "#FA383Dd9",
        },
      },
      error: {
        fg: {
          value: "var(--brand-color-danger-fg)",
          dark: "var(--brand-color-danger-fg)",
        },
        emphasis: {
          value: "var(--brand-color-danger-emphasis)",
          dark: "var(--brand-color-danger-emphasis)",
        },
        muted: {
          value: "var(--brand-color-danger-muted)",
          dark: "var(--brand-color-danger-muted)",
        },
        subtle: {
          value: "var(--brand-color-danger-subtle)",
          dark: "var(--brand-color-danger-subtle)",
        },
      },
      accent: {
        primary: {
          value: "var(--base-color-scale-green-7)",
          dark: "var(--base-color-scale-green-2)",
        },
        secondary: {
          value: "var(--base-color-scale-yellow-5)",
          dark: "var(--base-color-scale-yellow-3)",
        },
      },
      text: {
        default: {
          value: "var(--base-color-scale-black-0)",
          dark: "var(--base-color-scale-white-0)",
        },
        muted: {
          value: "var(--base-color-scale-gray-7)",
          dark: "var(--base-color-scale-gray-2)",
        },
        subtle: {
          value: "var(--brand-color-text-muted)",
          dark: "var(--brand-color-text-muted)",
        },
        onEmphasis: {
          value: "var(--base-color-scale-white-0)",
          dark: "var(--base-color-scale-black-0)",
        },
        emphasized: {
          value: "var(--base-color-scale-green-6)",
          dark: "var(--base-color-scale-green-3)",
        },
        link: {
          rest: {
            value: "var(--base-color-scale-blue-6)",
            dark: "var(--base-color-scale-blue-1)",
          },
          pressed: {
            value: "var(--base-color-scale-blue-8)",
            dark: "var(--base-color-scale-blue-1)",
          },
        },
        danger: {
          value: "#CF222E",
          dark: "#FF8182",
        },
        error: {
          value: "var(--brand-color-text-danger)",
          dark: "var(--brand-color-text-danger)",
        },
      },
      neutral: {
        emphasisPlus: {
          value: "var(--base-color-scale-gray-8)",
          dark: "var(--base-color-scale-gray-3)",
        },
        emphasis: {
          value: "var(--base-color-scale-gray-6)",
          dark: "var(--base-color-scale-gray-3)",
        },
        muted: {
          value: "#C4CCC6fe",
          dark: "#7C8980fe",
        },
        subtle: {
          value: "#E4EBE6fe",
          dark: "#7C8980ff",
        },
      },
      canvas: {
        default: {
          value: "var(--base-color-scale-white-0)",
          dark: "var(--base-color-scale-black-0)",
        },
        invert: {
          value: "var(--base-color-scale-black-0)",
          dark: "var(--base-color-scale-white-0)",
        },
        overlay: {
          value: "var(--base-color-scale-white-0)",
          dark: "var(--base-color-scale-gray-6)",
        },
        inset: {
          value: "var(--base-color-scale-gray-0)",
          dark: "var(--base-color-scale-black-0)",
        },
        subtle: {
          value: "var(--base-color-scale-gray-0)",
          dark: "var(--base-color-scale-gray-8)",
        },
        muted: {
          value: "#F9FBF9",
          dark: "#090D0A",
        },
      },
      border: {
        default: {
          value: "var(--base-color-scale-gray-4)",
          dark: "var(--base-color-scale-gray-5)",
        },
        muted: {
          value: "var(--base-color-scale-gray-1)",
          dark: "var(--base-color-scale-gray-7)",
        },
        subtle: {
          value: "var(--base-color-scale-gray-2)",
          dark: "var(--base-color-scale-gray-6)",
        },
      },
      focus: {
        value: "var(--base-color-scale-blue-5)",
        dark: "var(--base-color-scale-blue-3)",
      },
    },
  },
};
